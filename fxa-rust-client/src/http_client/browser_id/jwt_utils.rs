...
mod tests {
    ...
    fn test_rsa_generation() {
        ...
        let mock_public_key = rsa::create_public_key(
            mock_modulus.to_owned().unwrap(),
            mock_public_exponent.to_owned().unwrap(),
        ).unwrap();
        let mock_private_key = rsa::create_private_key(
            mock_modulus.to_owned().unwrap(),
            mock_public_exponent.to_owned().unwrap(),
            mock_private_exponent.to_owned().unwrap(),
        ).unwrap();
        let public_key_to_sign =
            rsa::create_public_key(n.to_owned().unwrap(), e.to_owned().unwrap()).unwrap();
        let private_key_to_sign = rsa::create_private_key(
            n.to_owned().unwrap(),
            e.to_owned().unwrap(),
            d.to_owned().unwrap(),
        ).unwrap();
        ...
        let assertion = create_assertion_full(
            &private_key_to_sign,
            &certificate,
            audience,
            issuer,
