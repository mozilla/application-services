...
pub fn assert_credit_cards_equiv(a: &CreditCard, b: &CreditCard) {
    ...
    assert_eq!(a.cc_number, b.cc_number, "cc_number mismatch");
    assert_eq!(
...
fn test_undecryptable_record_prevents_syncing(c0: &mut TestClient, c1: &mut TestClient) {
    ...
    // Simulate a corrupted record: reopen the store with a fresh key, under
    // which the existing record cannot be decrypted for upload.
...
fn test_scrub_undecryptable_records_for_remote_replacement(
    ...
    assert!(c0_scrubbed_record.cc_number.is_empty());
