...
void fairy_bridge_backend_c_init() { }
...
void fairy_bridge_backend_c_send_request(fairy_bridge::Request* request, fairy_bridge::Result* result) {
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, request->settings.timeout);
    curl_easy_setopt(curl, CURLOPT_MAXREDIRS, request->settings.redirect_limit);
