...
impl TabsStorage {
    ...
    fn filter_pending_remote_tabs(&mut self, crts: Vec<ClientRemoteTabs>) -> Vec<ClientRemoteTabs> {
        ...
        let pending_commands_result: Result<Vec<(String, RemoteCommand)>> = conn.query_rows_and_then_cached(
            "SELECT device_id, command, url
             FROM remote_tab_commands
             WHERE command IN (:command_close_tab, :command_close_inactive)",
            rusqlite::named_params! { ":command_close_tab": CommandKind::CloseTab, ":command_close_inactive": CommandKind::CloseAllInactiveTabs },
            |row| {
                let device_id = row.get::<_, String>(0)?;
                let kind = row.get::<_, CommandKind>(1)?;
                let command = match kind {
                    CommandKind::CloseTab => RemoteCommand::CloseTab { url: row.get::<_, String>(2)? },
                    CommandKind::CloseAllInactiveTabs => RemoteCommand::CloseAllInactiveTabs,
                };
                Ok((device_id, command))
            },
        );
        let pending_commands = match pending_commands_result {
            Ok(pending_commands) => pending_commands,
            Err(e) => {
                error_support::report_error!(
                    "filter_pending_remote_commands",
        ...
        // Run over the result to build the commands in different structures:
        // * A hash map of all urls which have been requested to be closed.
        let mut pending_closures: HashMap<String, Vec<String>> = HashMap::new();
        // * A simple vec of devices we want to "close all inactive" - building this in to a hashset seems a little over the top.
        let mut pending_close_inactive = Vec::new();
        for (device_id, command) in pending_commands {
            match command {
                RemoteCommand::CloseTab { url } => {
                    pending_closures.entry(device_id).or_default().push(url)
                }
                RemoteCommand::CloseAllInactiveTabs => pending_close_inactive.push(device_id),
            }
        }
        ...
        let filtered_crts: Vec<ClientRemoteTabs> = crts
            .into_iter()
            .map(|mut crt| {
                let hide_inactive = pending_close_inactive.contains(&crt.client_id);
                crt.remote_tabs.retain(|tab| {
                    (!hide_inactive || !tab.inactive)
                        && !pending_closures
                            .get(&crt.client_id)
...
impl TabsStorage {
    ...
    pub fn add_remote_tab_command_at(
        ...
        let url = match command {
            RemoteCommand::CloseTab { url } => Some(url),
            RemoteCommand::CloseAllInactiveTabs => None,
        };
        ...
        let result = conn.query_rows_and_then_cached(
            ...
            |row| -> Result<_> {
                ...
                Ok(Some(match command {
                    ...
                    CommandKind::CloseAllInactiveTabs => PendingCommand {
                        device_id: row.get::<_, String>(0)?,
                        command: RemoteCommand::CloseAllInactiveTabs,
                        time_requested: row.get::<_, Timestamp>(3)?,
                        time_sent: row.get::<_, Option<Timestamp>>(4)?,
                    },
                }))
    ...
    /// Removes any pending "close tabs" commands that are either:
    ///
    /// 1. REMOTE_COMMAND_TTL_MS older than when the command was queued, or...
    /// 2. The target device's incoming tabs reflect that the command was acted on
    ///    (eg, for tab close commands; does not have those tabs anymore;
    ///    for close inactive commands, has no inactive tabs).
    pub fn remove_old_pending_closures(
        ...
        // Insert new remote tabs into a couple of temporary tables.
        // We could make this a single table using a command and optional URL (ie,
        // like how we store the command itself) but that's still slightly messy,
        // but might make sense in the future as this evolves.
        conn.execute_batch(
            "
            CREATE TEMP TABLE IF NOT EXISTS new_remote_tabs (device_id TEXT, url TEXT);
            DELETE FROM new_remote_tabs;
            CREATE TEMP TABLE IF NOT EXISTS new_remote_tabs_without_inactive (device_id TEXT);
            DELETE FROM new_remote_tabs_without_inactive",
        )?;
        ...
        for (record, _) in new_remote_tabs.iter() {
            ...
            for tab in &record.tabs {
                ...
                saw_inactive |= tab.inactive;
            }
            if !saw_inactive {
                conn.execute(
                    "INSERT INTO new_remote_tabs_without_inactive (device_id) VALUES (?)",
                    rusqlite::params![fxa_id],
                )?;
            }
        }
        ...
        let delete_sql = "
         DELETE FROM remote_tab_commands
         WHERE
            (device_id IN (SELECT device_id from new_remote_tabs) AND command = :command_close_tab)
         AND
         (
            url NOT IN (
        ...
        // Delete pending "close inactive" commands
        let delete_inactive_sql = "
         DELETE FROM remote_tab_commands
         WHERE
            command = :command_close_inactive_tabs
         AND
            (device_id IN (SELECT device_id from new_remote_tabs INTERSECT SELECT device_id from new_remote_tabs_without_inactive))
        ";
        conn.execute(
            delete_inactive_sql,
            rusqlite::named_params! {
                ":command_close_inactive_tabs": CommandKind::CloseAllInactiveTabs,
            },
        )?;
        log::info!(
            "deleted {} pending inactive-tab closures because they were not in the new tabs",
            conn.changes()
        );
        ...
        tx.commit()?;
        conn.execute_batch(
            "DROP TABLE new_remote_tabs; DROP TABLE new_remote_tabs_without_inactive;",
        )?;
        Ok(())
    }
}
...
impl AsRef<CommandKind> for RemoteCommand {
    ...
    fn as_ref(&self) -> &CommandKind {
        match self {
            ...
            RemoteCommand::CloseAllInactiveTabs => &CommandKind::CloseAllInactiveTabs,
        }
    }
}
...
impl FromSql for CommandKind {
    fn column_result(value: ValueRef<'_>) -> FromSqlResult<Self> {
        Ok(match value.as_i64()? {
            ...
            1 => CommandKind::CloseAllInactiveTabs,
...
mod tests {
    ...
    fn test_add_pending_dupe_simple() {
        ...
        assert!(storage
            .add_remote_tab_command("device-1", &command)
            .expect("should work"));
        let command = RemoteCommand::CloseAllInactiveTabs;
        assert!(storage
            .add_remote_tab_command("device-1", &command)
            .expect("should work"));
        assert!(!storage
            .add_remote_tab_command("device-1", &command)
            .expect("should work but indicate it was already queued."));
    }
    ...
    fn test_remote_tabs_filters_pending_closures() {
            ...
            TabsSQLRecord {
                ...
                record: TabsRecord {
                    ...
                    tabs: vec![
                        ...
                        TabsRecordTab {
                            title: "the inactive title".to_string(),
        ...
        // Device 2 had 4 tabs open and we remotely closed 2, so we expect 2 tabs returned, including the inactive
        assert_eq!(remote_tabs[1].client_id, "device-2");
        assert_eq!(remote_tabs[1].remote_tabs.len(), 2);
        assert_eq!(
        ...
        // now ask for all inactive tabs to be closed.
        storage
            .add_remote_tab_command("device-2", &RemoteCommand::CloseAllInactiveTabs)
            .unwrap();
        ...
        // The "inactive" tab should no longer be returned.
        assert_eq!(remote_tabs[1].client_id, "device-2");
    ...
    #[test]
    fn test_remove_pending_closures_inactive() {
        env_logger::try_init().ok();
        let mut storage =
            TabsStorage::new_with_mem_path("test_remove_old_pending_closures_inactive");
        let db = storage.open_if_exists().unwrap().unwrap();
        ...
        "INSERT INTO remote_tab_commands (device_id, command, url, time_requested) VALUES (:device_id, :command, :url, :time_requested)",
        rusqlite::named_params! {
            ":command": CommandKind::CloseAllInactiveTabs,
            ":device_id": "device-recent",
            ":time_requested": now_ms,
        },
    ).unwrap();

        // Verify initial state has a pending closure.
        let before_count: i64 = db
            .query_row("SELECT COUNT(*) FROM remote_tab_commands", [], |row| {
                row.get(0)
            })
        ...
        // Simulate incoming data that continues to include an 'inactive' tab.
        let new_records = vec![(
            TabsRecord {
                id: "device-recent".to_string(),
                client_name: "".to_string(),
        ...
        let after_count: i64 = reopen_db
            ...
            .unwrap();
        assert_eq!(after_count, 1); // The entry should remain

        // Now the next sync happens and there are no inactive tabs.
        let new_records = vec![(
            TabsRecord {
                id: "device-recent".to_string(),
                client_name: "".to_string(),
        ...
        let after_count: i64 = reopen_db
            ...
            .unwrap();
        assert_eq!(after_count, 0); // The pending command should be removed.
    }
