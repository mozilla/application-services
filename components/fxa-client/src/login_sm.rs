...
use crate::{errors::*, http_client::session_token, util::Xorable, Config};
use serde_derive::*;
use std::sync::Arc;

pub struct LoginStateMachine<'a> {
    config: &'a Config,
    client: Arc<dyn session_token::FxASessionTokenClient>,
}
    ...
    fn handle_ready_for_key_state<F: FnOnce(ReadyForKeysState) -> LoginState>(
        ...
        match resp {
            Ok(resp) => {
                ...
                log::info!("Unwrapped keys response.  Transition to Married.");
                let sync_key = session_token::derive_sync_key(&kb);
                let xcs = session_token::compute_client_state(&kb);
                LoginState::Married(MarriedState {
...
impl MarriedState {
    pub fn sync_key(&self) -> &[u8] {
        &self.token_and_keys.sync_key
    }
    pub fn xcs(&self) -> &str {
        &self.token_and_keys.xcs
    }
}
...
impl LoginState {
    pub fn into_separated(self) -> Self {
        use self::LoginState::*;
        match self {
            Married(state) => Separated(state.token_and_keys.base),
