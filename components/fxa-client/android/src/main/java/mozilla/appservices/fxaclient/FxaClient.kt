...
class FxaClient(inner: FirefoxAccount, persistCallback: PersistCallback?) : AutoCloseable {
    ...
    /**
     * Close all tabs that are currently open and inactive on another device.
     *
     * This performs network requests, and should not be used on the main thread.
     *
     * @param targetDeviceId The ID of the device on which the tabs are
     * currently open.
     */
    fun closeAllInactiveTabs(targetDeviceId: String) {
        withMetrics {
            this.inner.closeAllInactiveTabs(targetDeviceId)
        }
    }
