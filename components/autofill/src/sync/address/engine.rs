...
pub const LAST_SYNC_META_KEY: &str = "addresses_last_sync_time";
pub const GLOBAL_SYNCID_META_KEY: &str = "addresses_global_sync_id";
pub const COLLECTION_SYNCID_META_KEY: &str = "addresses_sync_id";

pub struct AddressEngine<'a> {
    pub db: &'a AutofillDb,
    pub interruptee: &'a dyn Interruptee,
}

impl<'a> AddressEngine<'a> {}

impl<'a> SyncEngine for AddressEngine<'a> {
    #[inline]
    ...
    fn apply_incoming(
        ...
        assert_eq!(inbound.len(), 1, "addresses only requests one item");
    ...
    fn wipe(&self) -> anyhow::Result<()> {
        log::warn!("addresses doesn't implement wipe because there isn't a valid use case for it");
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_address_engine_sync_finished() -> Result<()> {
        let db = new_mem_db();
        let address_engine = AddressEngine {
            db: &db,
        ...
        let last_sync = 24;
        let result =
            address_engine.sync_finished(ServerTimestamp::from_millis(last_sync), Vec::new());
        assert!(result.is_ok());
    ...
    #[test]
    fn test_address_engine_get_sync_assoc() -> Result<()> {
        let db = new_mem_db();
        let address_engine = AddressEngine {
            db: &db,
            interruptee: &db.begin_interrupt_scope(),
        ...
        let result = address_engine.get_sync_assoc();
        assert!(result.is_ok());
