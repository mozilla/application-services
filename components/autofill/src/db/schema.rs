...
mod tests {
    ...
    fn test_upgrade_version_5() {
        ...
        // The migrated database is fully writable: a card added after the
        // upgrade encrypts and reads back like any other.
        let added = add_credit_card(
            &upgraded,
            card(
                &encrypt_str(encdec.as_ref(), "5500005555555559").unwrap(),
                "5559",
            ),
        )
        .unwrap();
        assert_eq!(
            SecureCreditCardFields::decrypt(
                &get_credit_card(&upgraded, &added.guid)
                    .unwrap()
                    .cc_number_enc,
                encdec.as_ref(),
                added.guid.as_str(),
