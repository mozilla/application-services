...
impl SecureCreditCardFields {
    ...
    pub fn decrypt(
        ...
        let clearbytes = encdec
            .decrypt(ciphertext.as_bytes().to_vec())
            .map_err(|e| {
        ...
        let cleartext = String::from_utf8(clearbytes).map_err(|e| {
            Error::DecryptionFailed(format!("{e} (decrypting {guid}: data not utf8)"))
        })?;
...
mod tests {
    ...
    use db_crypto::ManagedEncryptorDecryptor;
    ...
    fn test_a_bare_number_decrypts_as_the_number() {
        let encdec = encdec();
        let stored = crate::sync::credit_card::encrypt_str(&encdec, "4111111111117629").unwrap();
        assert_eq!(
            SecureCreditCardFields::decrypt(&stored, &encdec, "test-guid").unwrap(),
