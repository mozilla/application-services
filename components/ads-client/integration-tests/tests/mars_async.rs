...
fn test_contract_multi_ad_type_prod_async() {
    ...
    let result = client.prefetch_ads(
        vec![MozAdsPlacementRequest {
            iab_content: None,
            placement_id: placement_image_id.clone().into(),
        }],
        vec![MozAdsPlacementRequestWithCount {
            count: 4,
            iab_content: None,
            placement_id: placement_spoc_id.clone().into(),
        }],
        vec![MozAdsPlacementRequest {
            iab_content: None,
            placement_id: placement_tile_id.clone().into(),
        }],
        None,
    );
