...
Returns a new `MozAdsClientBuilder` for configuring the ads client. Use the builder's setter methods to configure the client, then call `.build()` to create the `MozAdsClient` instance.

**Example:**
```rust
let client = MozAdsClient::new()
