...
impl HttpCacheBuilder {
    ...
    #[allow(dead_code)]
    pub fn db_path(self, db_path: impl Into<PathBuf>) -> Self {
        Self {
            db_path: db_path.into(),
            ..self
        }
    ...
    #[allow(dead_code)]
    pub fn max_size(self, max_size: ByteSize) -> Self {
        Self {
...
mod tests {
    ...
    fn test_cache_builder_with_defaults() {
        let builder = HttpCacheBuilder::new("test.db".to_string(), None, None);
    ...
    fn test_cache_builder_valid_custom() {
        let builder = HttpCacheBuilder::new(
            "custom.db".to_string(),
            Some(ByteSize::b(1024)),
    ...
    fn test_validation_empty_db_path() {
        let builder = HttpCacheBuilder::new("   ".to_string(), None, None);
    ...
    fn test_validation_max_size_too_small() {
        let builder = HttpCacheBuilder::new("test.db".to_string(), Some(ByteSize::b(512)), None);
    ...
    fn test_validation_max_size_too_large() {
        let builder = HttpCacheBuilder::new(
            "test.db".to_string(),
            Some(ByteSize::b(2 * 1024 * 1024 * 1024)),
            None,
        );
    ...
    fn test_validation_max_size_boundaries() {
        let builder_min = HttpCacheBuilder::new("test.db".to_string(), Some(MIN_CACHE_SIZE), None);
        assert!(builder_min.validate().is_ok());

        let builder_max = HttpCacheBuilder::new("test.db".to_string(), Some(MAX_CACHE_SIZE), None);
        assert!(builder_max.validate().is_ok());
    }

    #[test]
    fn test_validation_ttl_too_small() {
        let builder =
            HttpCacheBuilder::new("test.db".to_string(), None, Some(Duration::from_secs(0)));
    ...
    fn test_validation_ttl_boundaries() {
        let builder_min = HttpCacheBuilder::new("test.db".to_string(), None, Some(MIN_TTL));
        assert!(builder_min.validate().is_ok());

        let builder_max = HttpCacheBuilder::new("test.db".to_string(), None, Some(MAX_TTL));
        assert!(builder_max.validate().is_ok());
    }
    ...
    #[test]
    fn test_build_with_validation_error() {
        let builder = HttpCacheBuilder::new(
            "".to_string(),
            Some(ByteSize::b(1024)),
            Some(Duration::from_secs(60)),
