...
pub fn upgrade_from(db: &Connection, from: u32) -> rusqlite::Result<()> {
    ...
    match from {
        ...
        17 => {
            // Drop the CHECK and FOREIGN KEY constraints on `moz_bookmarks`;
            // schemas >= 18 enforce constraints using custom TEMP triggers with
            // more informative error messages.

            // SQLite doesn't support `ALTER TABLE DROP CONSTRAINT`, so
            // we rewrite the schema for `moz_bookmarks` instead.
            ...
            db.pragma_update(None, "ignore_check_constraints", "OFF")?;
        }
