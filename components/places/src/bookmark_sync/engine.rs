...
use crate::db::{GlobalChangeCounterTracker, MutexPlacesDb, PlacesDb};
use crate::error::*;
...
fn update_local_items_in_places<'t>(
    ...
    apply_remote_items(db, interrupt_scope, now)?;
...
pub(crate) fn update_frecencies(db: &PlacesDb, interrupt_scope: &InterruptScope) -> Result<()> {
    let mut tx = db.begin_transaction()?;
...
impl SyncEngine for BookmarksSyncEngine {
    ...
    fn apply_incoming(
        ...
        // Merge.
        let mut merger = Merger::with_telemetry(&conn, &self.interrupt_scope, timestamp, telem);
        merger.merge()?;

        // Finally, stage outgoing items.
        let outgoing = fetch_outgoing_records(&conn, &self.interrupt_scope, timestamp)?;
        Ok(outgoing)
    }
    ...
    fn sync_finished(
        ...
        let conn = self.db.lock();
        push_synced_items(&conn, &self.interrupt_scope, new_timestamp, records_synced)?;
        update_frecencies(&conn, &self.interrupt_scope)?;
...
impl<'a> Merger<'a> {
    ...
    pub(crate) fn merge(&mut self) -> Result<()> {
        ...
        self.prepare()?;
        let result = self.merge_with_driver(&driver, &MergeInterruptee(self.interrupt_scope));
...
impl<'a> dogear::Store for Merger<'a> {
    ...
    fn apply(&mut self, root: MergedRoot<'_>) -> Result<()> {
        let ops = root.completion_ops_with_signal(&MergeInterruptee(self.interrupt_scope))?;
        ...
        update_local_items_in_places(self.db, self.interrupt_scope, self.local_time, &ops)?;
...
mod tests {
    ...
    fn create_sync_engine(api: &PlacesApi) -> BookmarksSyncEngine {
        BookmarksSyncEngine::new(api.get_sync_connection().unwrap(), InterruptScope::new())
    }
    ...
    fn apply_incoming(
        ...
        push_synced_items(
            &syncer,
            &engine.interrupt_scope,
            remote_time,
            uploaded_guids.clone(),
        )
