...
/// Version 3: addition of `timeLastPotentiallyVulnerablePasswordSet` and `timeLastBreachAlertDismissed`.
...
const COMMON_SQL: &str = "
    ...
    timeLastPotentiallyVulnerablePasswordSet    INTEGER,
...
fn upgrade_from(db: &Connection, from: i64) -> Result<()> {
    ...
    match from {
        ...
        2 => Ok(db.execute_batch(
            "ALTER TABLE loginsL ADD timeLastPotentiallyVulnerablePasswordSet INTEGER;
        ALTER TABLE loginsM ADD timeLastPotentiallyVulnerablePasswordSet INTEGER;
...
mod tests {
    ...
    fn test_upgrade() {
        ...
        db.execute_batch("SELECT timeLastPotentiallyVulnerablePasswordSet FROM loginsL")
            .unwrap();
        db.execute_batch("SELECT timeLastPotentiallyVulnerablePasswordSet FROM loginsM")
            .unwrap();
