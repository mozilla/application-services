...
mod test {
    ...
    fn test_deletions_and_lists()
    {
        ...
        // Test that we only have a list of one since only one item exists for this hostname
        assert_eq!(list.len(), 1);
        assert_eq!(list[0], b_from_db);
        // Test that if we try to get an item by a hostname that does not exist we will get
        // an empty list
        let list = engine
