...
mod test {
    ...
    fn test_logins_and_updates()
    {
        ...
        let b_from_db = engine
            ...
            .expect("b to exist");        assert_logins_equiv(
            &b_from_db,
            &Login {
        ...
        assert_eq!(b_from_db.times_used, 1);        //now we are going to update item b - ben and ryan
        ...
        let b_after_update = engine
            .get(&b_id)
            .expect("Not to error getting b")
            .expect("b to exist");        //checking logins/times are still good after updating - ben and ryan
    ...
    fn test_general()
    {
        ...
        let a_id = engine.add(a.clone()).expect("added a");
        let b_id = engine.add(b.clone()).expect("added b");        //Make sure our ID's are what we initialized them to, and also test
        //case where we dont initialize an ID - ben and ryan
        assert_eq!(a_id, a.guid);
        assert_ne!(b_id, b.guid, "Should generate guid when none provided");        //Get item a's ID, make sure no errors getting a and for a's ID to exist - ben and ryan
        let a_from_db = engine
            .get(&a_id)
            .expect("Not to error getting a")
            .expect("a to exist");
        ...
        assert_eq!(list, expect);        //testing if we can successfully delete an id, but we can still get the item after the delete - ben and ryan
        engine.delete(&a_id).expect("Successful delete");
        assert!(engine
            .get(&a_id)
            .expect("get after delete should still work")
            .is_none());        //checking that our list of items is correct from here on - ben and ryan
        let list = engine.list().expect("Grabbing list to work");
        assert_eq!(list.len(), 1);
        assert_eq!(list[0], b_from_db);        let list = engine
