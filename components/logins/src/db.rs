...
impl LoginDb {
    ...
    pub fn verify_logins(&self, encdec: &dyn EncryptorDecryptor) -> Result<bool> {
        let corrupted_logins = self.get_all()?
            .into_iter()
        ...
        Ok(self.delete_local_records_for_remote_replacement(ids).is_ok())
    }

    fn delete_local_records_for_remote_replacement(&self, ids: Vec<&str>) -> Result<()> {
        let tx = self.unchecked_transaction_imm()?;
