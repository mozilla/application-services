...
impl NimbusTargetingHelper {
    ...
    pub fn evaluate_jexl_raw_value(&self, expr: String) -> Result<Value> {
        cfg_if::cfg_if! {
            if #[cfg(feature = "stateful")] {
...
pub fn jexl_eval_raw<Context: serde::Serialize>(
    ...
    match evaluator.eval_in_context(expression_statement, context) {
        Ok(v) => Ok(v),
        Err(err) => Err(NimbusError::EvaluationError(err.to_string())),
    }
}
