...
fn test_ios_rollout_experiment() -> Result<()> {
    ...
    let enrollment = evolver
        .evolve_enrollment::<Experiment>(true, None, Some(exp), None, &mut events, &None)?
        .unwrap();
...
fn test_evolver_new_experiment_globally_opted_out() -> Result<()> {
    ...
    let mut events = vec![];
    let enrollment = evolver
        .evolve_enrollment::<Experiment>(false, None, Some(&exp), None, &mut events, &None)?
        .unwrap();
    assert!(matches!(
        enrollment.status,
        EnrollmentStatus::NotEnrolled {
...
fn test_evolve_enrollments_with_coenrolling_features() -> Result<()> {
    ...
    let (enrollments, _) =
        evolver.evolve_enrollment_recipes(true, &no_experiments, &all_experiments, &[], &None)?;
...
fn test_evolver_existing_experiment_has_no_enrollment() {
    ...
    let ids = no_coenrolling_features();
    let mut evolver = enrollment_evolver(&mut th, &aru, &ids);
    let res = evolver.evolve_enrollment(true, Some(&exp), Some(&exp), None, &mut vec![], &None);
    assert!(res.is_err());
}
...
fn test_evolver_no_experiments_no_enrollment() {
    ...
    let mut evolver = enrollment_evolver(&mut th, &aru, &ids);
    evolver
        .evolve_enrollment::<Experiment>(true, None, None, None, &mut vec![], &None)
        .unwrap();
}
...
fn test_enrollment_enrolled_explicit_opt_out() {
    ...
    let enrollment = existing_enrollment.on_explicit_opt_out(&mut events, &None);
...
#[cfg(feature = "stateful")]
#[test]
fn test_on_revert_to_previous_state_with_gecko_prefs() {
    let exp = get_test_experiments()[0].clone();
    let existing_enrollment = ExperimentEnrollment {
        slug: exp.slug,
        status: EnrollmentStatus::Enrolled {
    ...
    let original_previous_state: PreviousState = PreviousState::GeckoPref(PreviousGeckoPrefState {
        original_values: vec![OriginalGeckoPref::from(&pref_state_1)],
        feature_id: "feature-id".to_string(),
        variable: "variable".to_string(),
    });
    let enrollment = existing_enrollment.on_add_state(original_previous_state);
    ...
    } = &enrollment.status
    {
        ...
        let PreviousState::GeckoPref(gecko_pref_state) = previous_state;
        assert_eq!(1, original_prefs_stored.len());
        assert_eq!(gecko_pref_state.original_values, original_prefs_stored);
    }
}
...
fn test_will_pref_experiment_change_no_change() {
    ...
    let original_enrollment = ExperimentEnrollment {
        ...
        status: EnrollmentStatus::Enrolled {
            ...
            reason: EnrolledReason::Qualified,
            #[cfg(feature = "stateful")]
            previous_state: Some(original_previous_state),
        },
    };
