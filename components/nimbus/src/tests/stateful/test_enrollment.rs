...
use crate::{
    ...
    stateful::{
        behavior::EventStore,
        enrollment::{
            get_enrollments, get_experiments_user_participation, get_rollouts_user_participation,
            opt_in_with_branch, opt_out, reset_telemetry_identifiers,
            set_experiments_user_participation, set_rollouts_user_participation,
        },
...
fn test_global_opt_out() -> Result<()> {
    ...
    set_experiments_user_participation(&db, &mut writer, false)?;
    ...
    set_experiments_user_participation(&db, &mut writer, true)?;
...
fn test_experiments_opt_out_with_rollouts_opt_in() -> Result<()> {
    ...
    set_experiments_user_participation(&db, &mut writer, false)?;
    set_rollouts_user_participation(&db, &mut writer, true)?;

    // Verify flags are set correctly
    let exp_participation = get_experiments_user_participation(&db, &writer)?;
    let rollouts_participation = get_rollouts_user_participation(&db, &writer)?;
...
fn test_rollouts_opt_out_with_experiments_opt_in() -> Result<()> {
    ...
    set_experiments_user_participation(&db, &mut writer, true)?;
    set_rollouts_user_participation(&db, &mut writer, false)?;
