...
use crate::{
    enrollment::{
        ...
        PreviousGeckoPrefState, PreviousState,
    },
    error::{info, Result},
    json::PrefValue,
    ...
    stateful::{
        ...
        enrollment::get_previous_state_for_experiment,
        gecko_prefs::{
...
#[test]
fn test_register_previous_state() -> Result<()> {
    let metrics = TestMetrics::new();
    let temp_dir = tempfile::tempdir()?;
    let app_context = AppContext {
        app_name: "fenix".to_string(),
    ...
    let active_experiments = client.get_active_experiments()?;
    assert_eq!(active_experiments.len(), 1);
    assert_eq!(active_experiments[0].slug, experiment_slug);
    ...
    let previous_state = PreviousGeckoPrefState {
    ...
    let original_previous_state: PreviousGeckoPrefState = previous_state.clone();

    client.register_previous_state(
    ...
    assert!(matches!(
        ...
        EnrollmentStatus::Enrolled { previous_state, .. }
            if previous_state == Some(PreviousState::GeckoPref(original_previous_state))
    ));
...
fn register_previous_gecko_pref_states() -> Result<()> {
    ...
    let gecko_pref_state_2 = GeckoPrefState::new("some.pref", Some(PrefBranch::Default))
        .with_gecko_value(json!("some-gecko-value"))
        .with_enrollment_value(PrefEnrollmentData {
    ...
    let gecko_pref_states = vec![gecko_pref_state_1, gecko_pref_state_2];
    ...
    let previous_state_1 = PreviousGeckoPrefState {
        original_values: vec![OriginalGeckoPref {
            pref: "some.pref".into(),
            branch: PrefBranch::Default,
            value: Some(serde_json::Value::String(String::from("some-gecko-value"))),
    ...
    assert!(matches!(
        ...
        EnrollmentStatus::Enrolled { previous_state, .. }
            if previous_state == Some(PreviousState::GeckoPref(previous_state_1.clone()))
    ));

    let previous_state_1_using_get = client.get_previous_state(experiment_slug_1.to_string());

    assert_eq!(
        Some(PreviousState::GeckoPref(previous_state_1)),
        previous_state_1_using_get.unwrap()
    );
...
#[test]
fn test_add_previous_state_for_experiment() -> Result<()> {
    let metrics = TestMetrics::new();
    let temp_dir = tempfile::tempdir()?;
    let app_context = AppContext {
        app_name: "fenix".to_string(),
    ...
    let db = client.db()?;
    let mut writer = db.write()?;
    NimbusClient::add_previous_state_for_experiment(
        db,
        &mut writer,
        experiment_slug,
        PreviousState::GeckoPref(original_previous_state.clone()),
    )?;
    let enrollments: Vec<ExperimentEnrollment> =
        db.get_store(StoreId::Enrollments).collect_all(&writer)?;
    ...
    assert!(matches!(
        ...
        EnrollmentStatus::Enrolled { previous_state, .. }
            if previous_state == Some(PreviousState::GeckoPref(original_previous_state.clone()))
    ));

    let reader_result = get_previous_state_for_experiment(db, &writer, experiment_slug);

    assert_eq!(
        reader_result.unwrap(),
        Some(PreviousState::GeckoPref(original_previous_state.clone()))
    );
    Ok(())
}
