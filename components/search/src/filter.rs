...
impl From<JSONEngineUrl> for SearchEngineUrl {
    fn from(url: JSONEngineUrl) -> Self {
        Self {
            base: url.base.unwrap_or_default().to_string(),
...
impl JSONEngineUrls {
    fn maybe_merge_urls(
        base_url: &Option<JSONEngineUrl>,
        variant_url: &Option<JSONEngineUrl>,
    ) -> Option<JSONEngineUrl> {
        match (base_url, variant_url) {
            (Some(base), Some(variant)) => Some(base.clone().merge(variant.clone())),
            (None, Some(variant)) => Some(variant.clone()),
            _ => base_url.clone(),
        }
    }

    fn merge(self, other: JSONEngineUrls) -> Self {
        Self {
            search: self.search.merge(other.search),
            suggestions: JSONEngineUrls::maybe_merge_urls(&self.suggestions, &other.suggestions),
            trending: JSONEngineUrls::maybe_merge_urls(&self.trending, &other.trending),
        }
    }
}
...
impl SearchEngineDefinition {
    pub(crate) fn from_configuration_details(
        ...
        let urls: JSONEngineUrls = match variant.urls {
            Some(urls) => base.urls.merge(urls),
            None => base.urls,
        };
        ...
        SearchEngineDefinition {
            ...
            partner_code: variant
                .partner_code
                .unwrap_or_else(|| base.partner_code.unwrap_or_default()),
