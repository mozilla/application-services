...
impl FilterRecords for Vec<RemoteSettingsRecord> {
    fn filter_records(
        &self,
        user_environment: &SearchUserEnvironment,
    ) -> Result<FilterRecordsResult, Error> {
...
impl FilterRecords for Vec<JSONSearchConfigurationRecords> {
    fn filter_records(
        &self,
        user_environment: &SearchUserEnvironment,
    ) -> Result<FilterRecordsResult, Error> {
...
pub(crate) fn filter_engine_configuration_impl(
    user_environment: SearchUserEnvironment,
    configuration: &impl FilterRecords,
    ...
    filtered_result.map(|result| {
        let (default_engine_id, default_private_engine_id) = determine_default_engines(
            &result.reduced_engines,
            result.default_engines_record,
            &user_environment,
        );

        let mut engines = result.reduced_engines.clone();
