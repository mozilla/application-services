...
/// Mutex to prevent concurrent fetches of the same gateway config
static FETCH_MUTEX: Lazy<Mutex<()>> = Lazy::new(|| Mutex::new(()));
...
pub async fn fetch_gateway_config(gateway_host: &str) -> Result<Vec<u8>> {
    ...
    crate::trace!("Starting gateway config fetch for: {}", gateway_host);

    // Quick cache check without lock
    {
        let cache = CONFIG_CACHE.read();
        if let Some(cached) = cache.get(gateway_host) {
            if cached.expires_at > SystemTime::now() {
                let remaining_secs = cached
                    .expires_at
                    .duration_since(SystemTime::now())
                    .unwrap_or(Duration::from_secs(0))
                    .as_secs();
                crate::trace!(
                    "Using cached config for gateway: {} (expires in {}s)",
                    gateway_host,
                    remaining_secs
                );
                return Ok(cached.config_data.clone());
            } else {
                crate::trace!("Cached config for gateway {} has expired", gateway_host);
            }
        } else {
            crate::trace!("No cached config found for gateway: {}", gateway_host);
        }
    }

    // Prevent concurrent fetches - only one fetch per gateway at a time
    crate::trace!("Acquiring fetch mutex for gateway: {}", gateway_host);
    let _guard = FETCH_MUTEX.lock().await;
    crate::trace!("Fetch mutex acquired for gateway: {}", gateway_host);
    ...
    // Double-check cache after acquiring mutex (another request might have populated it)
    {
        ...
        if let Some(cached) = cache.get(gateway_host) {
            if cached.expires_at > SystemTime::now() {
                crate::trace!(
                    "Using cached config for gateway (after mutex): {} (another thread populated cache)",
                    gateway_host
                );
                return Ok(cached.config_data.clone());
            }
    ...
    crate::trace!("Fetching fresh config for gateway: {}", gateway_host);

    // Construct gateway config URL - assuming HTTPS
    let gateway_url = format!("https://{}", gateway_host);
    let config_url = Url::parse(&gateway_url)?.join("ohttp-configs")?;
    crate::trace!("Gateway config URL: {}", config_url);
    ...
    crate::trace!(
        "Sending config fetch request to gateway (timeout: {}ms)",
        settings.timeout
    );
    ...
    crate::trace!(
        "Gateway config fetch completed in {:?}, status: {}",
        fetch_duration,
        response.status
    );
    ...
    if !response.is_success() {
        crate::error!(
            "Failed to fetch config from {}: HTTP {} (took {:?})",
            config_url,
            response.status,
            fetch_duration
        );
    ...
    if config_data.is_empty() {
        crate::error!("Gateway {} returned empty config data", gateway_host);
    ...
    crate::trace!(
        "Successfully fetched gateway config: {} bytes in {:?}",
        config_data.len(),
        fetch_duration
    );

    // Cache the config for 1 hour
    let cached_config = CachedGatewayConfig {
...

    {
        ...
        cache.insert(gateway_host.to_string(), cached_config);
        crate::trace!(
            "Gateway config cached for {} (expires in 1 hour)",
            gateway_host
        );
    }
