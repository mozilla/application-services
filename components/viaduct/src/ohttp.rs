...
use crate::{Request, Response, Result, ViaductError, Method, Headers};
...
pub fn configure_ohttp_channel(channel: String, config: OhttpConfig) -> Result<()> {
    crate::info!("Configuring OHTTP channel '{}' with relay: {}", channel, config.relay_url);
...
pub async fn fetch_relay_config(relay_url: &str) -> Result<Vec<u8>> {
    ...
    if !response.is_success() {
        return Err(ViaductError::OhttpConfigFetchFailed(
            format!("Failed to fetch config from {}: HTTP {}", config_url, response.status)
        ));
    }
...
pub async fn process_ohttp_request(
    ...
    crate::info!("Got OHTTP config - relay: {}, target: {}", config.relay_url, config.target_host);
    ...
    let ohttp_session = as_ohttp_client::OhttpSession::new(&relay_config_data)
        .map_err(|e| ViaductError::OhttpRequestError(
            format!("Failed to create OHTTP session: {}", e)
        ))?;
    ...
    crate::info!("Request details: {} {} {}://{}{}",
                method, path_and_query, scheme, authority, path_and_query);
    ...
    let encrypted_request = ohttp_session
        .encapsulate(method, scheme, authority, &path_and_query, headers_map, &payload)
        .map_err(|e| ViaductError::OhttpRequestError(
            format!("Failed to encapsulate request: {}", e)
        ))?;
    crate::info!("Request encapsulated: {} bytes encrypted", encrypted_request.len());
    ...
    if !relay_response.is_success() {
        return Err(ViaductError::OhttpRequestError(
            format!("OHTTP relay returned error: HTTP {} - {}",
    ...
    if let Some(content_type) = relay_response.headers.get("content-type") {
        if content_type != "message/ohttp-res" {
            crate::warn!("OHTTP relay returned unexpected content-type: {}", content_type);
        }
    }

    // Decapsulate the encrypted response using as-ohttp-client
    crate::info!("Decapsulating response: {} bytes", relay_response.body.len());
    let ohttp_response = ohttp_session.decapsulate(&relay_response.body)
        .map_err(|e| ViaductError::OhttpResponseError(
            format!("Failed to decapsulate OHTTP response: {}", e)
        ))?;

    // Convert the as-ohttp-client response back to a viaduct Response using the public methods
    ...
    crate::info!("OHTTP request completed successfully for channel '{}': {} {}",
                 channel, final_response.status, final_response.url);
...
mod tests {
    ...
    fn test_headers_conversion() {
        ...
        assert_eq!(headers_back.get("Content-Type").unwrap(), "application/json");
