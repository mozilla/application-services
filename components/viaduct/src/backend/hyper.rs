...
use crate::{
    init_backend, Backend, ClientSettings, Error, Header, Method, Request, Response, Result,
};
...
#[uniffi::export]
pub fn init_backend_hyper() -> Result<(), Error> {
            ...
            .spawn(async move {
                ...
                if req_timeout == 0 {
                    ...
                    timeout(duration, req)
                        .await
                        .unwrap_or_else(|_| Err(Error::new_backend_error("Request timeout")))
                }
            })
            .await
        {
            Ok(result) => result,
            Err(e) => Err(Error::new_backend_error(format!(
...
async fn make_request_inner(
    ...
    while resp.status().is_redirection() {
        ...
        if settings.redirect_limit != 0 && redirect_count > settings.redirect_limit {
            return Err(Error::new_backend_error("Too many redirections"));
        }
        let Some(location) = resp.headers().get("location") else {
            return Err(Error::new_backend_error("location header missing"));
        };
