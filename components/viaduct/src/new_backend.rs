...
//
// We can transition from the old to new backend using the following steps:
//
// - Move applications over to using the new backend.  Components will continue to use the legacy
//   API, which is possible because we adapt the new backend to implement the old backend.
...
use crate::{
    backend as old_backend, settings::GLOBAL_SETTINGS, ClientSettings, Error, Request, Response,
    Result,
};
...
pub fn init_backend(backend: Arc<dyn Backend>) -> Result<()> {
    ...
    REGISTERED_BACKEND
        .set(backend)
        .map_err(|_| Error::BackendAlreadyInitialized)
}

pub fn get_backend() -> Result<&'static Arc<dyn Backend>> {
    REGISTERED_BACKEND.get().ok_or(Error::BackendNotInitialized)
}
