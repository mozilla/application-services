...
impl Storage {
    ...
    pub fn get_last_modified_timestamp(&self, collection_url: &str) -> Result<Option<u64>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT MAX(json_extract(data, '$.last_modified')) FROM records WHERE collection_url = ?",
        )?;

        let result: Option<u64> = stmt.query_row(params![collection_url], |row| row.get(0))?;
    ...
    /// to `set_records`.
    pub fn get_records(&self, collection_url: &str) -> Result<Option<Vec<RemoteSettingsRecord>>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare("SELECT data FROM records WHERE collection_url = ?")?;
        let mut rows = stmt.query(params![collection_url])?;
    ...
    pub fn get_attachment(
        ...
        let conn = self.conn.lock().unwrap();
        let mut stmt =
            conn.prepare("SELECT data FROM attachments WHERE id = ? AND collection_url = ?")?;
        let result: Option<Vec<u8>> = stmt
            .query_row(params![attachment_id, collection_url], |row| row.get(0))
            .optional()?;
    ...
    pub fn set_records(
        ...
        // Delete existing records for this collection_url
        tx.execute(
...
mod tests {
    ...
    fn test_storage_collection_url_isolation() -> Result<()> {
        ...
        // Get records for collection_url1
