...
impl Storage {
    ...
    /// to `set_records`.
    pub fn get_last_modified_timestamp(&self, _base_url: &str) -> Result<Option<u64>> {
        Ok(None)
    }
    ...
    /// Returns None if no records are stored or if `base_url` does not match the `base_url` passed
    /// to `set_records`.
    pub fn get_records(&self, _base_url: &str) -> Result<Option<Vec<RemoteSettingsRecord>>> {
        Ok(None)
    }
    ...
    pub fn set_records(&self, _base_url: &str, records: &[RemoteSettingsRecord]) -> Result<()> {
