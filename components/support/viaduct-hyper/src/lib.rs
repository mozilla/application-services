...
//! It's essentially the same as the builtin `dev` backend, except it also supports HTTPS.
//! The reason it exists is that so we can avoid the `hyper-tls` dependency in our main crates,
//! which causes dependency issues when vendored into `moz-central`.
//!
//! Because of those dependency issues, only use this for crates that aren't members of the moz-central workspace.
//! For example, CLI binaries or the ios megazord.
...
/// Set the viaduct backend to the `hyper`-based one.
///
/// This is mainly used for iOS and tests.
///
/// In the testing situation, we can ignore any errors.  Errors happen when multiple places try to
/// initialize the backend, which is expected during the tests.  The first one will take effect and
/// all others can be safely ignored.
#[uniffi::export]
