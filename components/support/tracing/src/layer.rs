...
pub fn register_target_event_sink(
    ...
    let targets = EventSinkSpecification {
    ...
    register_event_sink(targets, sink)
}
...
///
/// Returns an event sink ID that can be used to unregister the sink
pub fn register_min_level_event_sink(level: Level, sink: Arc<dyn EventSink>) -> EventSinkId {
    register_event_sink(
...
#[uniffi::export(name = "register_target_event_sink")]
pub fn register_target_event_sink_box(
    target: &str,
    level: Level,
    sink: Box<dyn EventSink>,
) -> EventSinkId {
    register_target_event_sink(target, level, sink.into())
}

#[uniffi::export(name = "register_min_level_event_sink")]
pub fn register_min_level_event_sink_box(level: Level, sink: Box<dyn EventSink>) -> EventSinkId {
