    ...
    S: tracing::Subscriber,
{
    fn callsite_enabled(&self, meta: &Metadata<'_>) -> Interest {
        if meta.fields().field("app_services").is_some() {
            // Event came from tracing_support's logging macros, which indicates this is an app_services crate.
            ...
            // Event came from outside app_services, we don't want to handle it.
