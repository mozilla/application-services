...
#[uniffi::export]
pub fn register_event_sink(target: &str, level: crate::Level, sink: Box<dyn EventSink>) {
    register_event_sink_arc(target, level, sink.into())
}

// the rust api still takes an Arc<> - this should die with callbacks...
pub fn register_event_sink_arc(target: &str, level: crate::Level, sink: Arc<dyn EventSink>) {
    // Callback interface means we get a `Box` instead of an `Arc<>`.
    ...
    fn on_event(
        ...
        if let Some(entry) = SINKS_BY_TARGET.read().get(prefix) {
            ...
            if level <= entry.level {
                ...
                // println!("DISPATCHING TRACING EVENT: {event:?}");
