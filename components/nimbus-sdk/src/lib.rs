...
pub mod error;
mod evaluator;
use nimbus_core::behavior::EventStore;
...
pub struct NimbusClient {
    ...
    db_path: PathBuf,
    event_store: Arc<Mutex<SDKEventStore>>,
}
...
impl NimbusClient {
    ...
    pub fn clear_events(&self) -> Result<()> {
        let mut event_store = self.event_store.lock().unwrap();
        event_store.clear_db(self.db()?)?;
        Ok(())
    }

    pub fn event_store(&self) -> Arc<Mutex<SDKEventStore>> {
...
impl NimbusTargetingHelper {
    fn new(context: Value, event_store: Arc<Mutex<SDKEventStore>>) -> Self {
        Self {
            context,
            event_store,
        }
